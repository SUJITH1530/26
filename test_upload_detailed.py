...
# Create a more realistic test image using PIL if available, otherwise a minimal JPEG
...
# Test upload with error details
try:
    with app.test_client() as client:
        ...
        if resp.status_code == 200:
            ...
            print(f"  - Vehicles detected: {data.get('vehicles', 0)}")
            print(f"  - Processed image: {data.get('processed_image')}")
            print(f"  - Has lanes data: {'lanes' in data}")
        else:
